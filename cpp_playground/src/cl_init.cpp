...
void	cl_compile_kernel(CLCore &cl, const char *filepath, const char *program_name)
{
	...
	free(kernel);
	// Compile
	err = clBuildProgram(cl.program, 1, &(cl.device), "-I ../cpp_playground/src/kernels -cl-fast-relaxed-math", NULL, NULL);
	if (err != CL_SUCCESS)
	{
		size_t len;
		char buffer[2048];
...
void	cl_compile_water_kernel(CLWaterCore &cl, cl_program &program_field, cl_kernel &kernel_field, const char *filepath, const char *program_name)
{
	...
	free(kernel);
	// Compile
	err = clBuildProgram(program_field, 1, &(cl.device), "-I ../cpp_playground/src/kernels -cl-fast-relaxed-math", NULL, NULL);
	if (err != CL_SUCCESS)
	{
		size_t len;
		char buffer[2048];
