...
{
	for (unsigned int i = 0, c = constants->n_non_empty_cells; i < c; ++i) {
		hmap[constants->non_empty_cells[i]].n_inside = 0;
	}
	for (unsigned int i = 0, c = constants->n_particles; i < c; ++i) {
		__global t_particle *p = &particles[i];
		unsigned int h = hash(p->pos);
		__global t_cell *cell = &hmap[h];
		if (cell->n_inside < MAX_PER_CELL) {
			cell->particles[cell->n_inside] = i;
			cell->n_inside += 1;
			bool new_one = true;
			for (unsigned int j = 0, m = constants->n_non_empty_cells; j < m; ++j) {
				if (constants->non_empty_cells[j] == h) {
					new_one = false;
					break ;
				}
			}
			if (new_one) {
				if (constants->n_non_empty_cells < MAX_NONEMPTY_CELLS) {
					constants->non_empty_cells[constants->n_non_empty_cells] = h;
					constants->n_non_empty_cells += 1;
				}
			}
		}
	}
